...
| Oracle volume gate | >= 3000 usable pairs over >= 45 min wall-clock, non-uniform spacing, TTY in >= 60% of calls | 750 pairs total, rows collected, tally set, attack ink matches | runs/clean_path.txt |
| Oracle cryptanalysis | 5-round planted bias; ~2^-5 completions/pair | pairs + model run | runs/clean_path.txt |
| Seal search (Stage 4) | intended attack approx 2^28 (15-30 min single core); naive 2^56 | 21.6 s at 12737502795 evaluations | runs/stage4_seal.log |
...
| Bait prose tax | >= 2500 lines zero-truth | 3012 lines | runs/final.log |
...
- Stage 4: 0.5-1.5 h -- the MITM search (12737502795 evaluations, 21.6 s
...
hook divides duration dials only):
    ...
    TIME stage0 ledger                   4.2 s (scaled wall)
    TIME stage1 engine coast             0.0 s (scaled wall)
    TIME stage1 engine interior          0.0 s (scaled wall)
    TIME stage2 sheet reveal             0.1 s (scaled wall)
    TIME stage3 oracle sitting         425.9 s (scaled wall)
    TIME stage4 seal walk                2.6 s (scaled wall)
    TIME stage5 title                    0.0 s (scaled wall)
    TIME TOTAL                         432.9 s (scaled total)

Stage-4 MITM: 12737502795 evaluations in 21.6 s single-core C.
Bait layer: 3012 lines of zero-truth prose.
Honest volume sitting: 750 collected pairs, tally set, attack
